...
class AdultParams_nonuai(object):
  ...
  PROTECTED_GROUPS = (
      'gender_Female', 'gender_Male', #'race_Black', 'race_White',
  )
...
def read_and_preprocess_adult_data_nonuai(joint_protected_groups=True):
  ...
  train_df = train_df_raw[AdultParams.CATEGORICAL_COLUMNS + AdultParams.CONTINUOUS_COLUMNS +
                          [AdultParams.LABEL_COLUMN]]
  test_df = test_df_raw[AdultParams.CATEGORICAL_COLUMNS + AdultParams.CONTINUOUS_COLUMNS +
                        [AdultParams.LABEL_COLUMN]]
  ...
  train_df, test_df = binarize_categorical_columns(
      train_df,
      test_df,
      categorical_columns=AdultParams.CATEGORICAL_COLUMNS + AdultParams.CONTINUOUS_COLUMNS)
...
def read_and_preprocess_adult_data_uai(
  ...
  for df in [train_df, test_df]:
    df[AdultParams.LABEL_COLUMN] = df['income'].apply(
        lambda x: '>50K' in x).astype(int)

  # Filter for the columns we want.
  desired_columns = list((AdultParams.LABEL_COLUMN,) +
  ...
  # the quantiles are.
  def quantilise_continuous_column(
      dataframes, continuous_column_name, num_quantiles):
    _, bins = pd.qcut(dataframes[0][continuous_column_name],
                      num_quantiles, retbins=True, labels=False)
  ...
  for column_name, num_quantiles in continuous_column_quantile_counts:
    quantilise_continuous_column([train_df, test_df],
                                 column_name, num_quantiles)
  for column_name, buckets in continuous_column_buckets:
    bucketise_continuous_column([train_df, test_df],
                                column_name, buckets)
  ...
  def binarise_categorical_columns(dataframes):
    ...
    columns = list(AdultParams.CATEGORICAL_COLUMNS +
                   AdultParams.CONTINUOUS_COLUMNS)
...
def read_and_preprocess_bank_data(
    ...
    test_set_fraction=0.2, rng_seed=42):
  ...
  def binarise_categorical_columns(df):
    return pd.get_dummies(
        df, columns=list(BankParams.CATEGORICAL_COLUMNS) + [
...
class CrimeParams(object):
  ...
  PROTECTED_GROUPS = (
      'racepctblack_cat_low', 'racepctblack_cat_high', 'racePctAsian_cat_low',
      'racePctAsian_cat_high', 'racePctWhite_cat_low', 'racePctWhite_cat_high',
...
def read_and_preprocess_crime_data():
  ...
  feature_names = [c for c in df_train.keys()  # pylint: disable=g-complex-comprehension
                   if c not in list(CrimeParams.EXCLUDED_COLUMNS) + [
                       CrimeParams.LABEL_COLUMN,
                       CrimeParams.REGRESSION_TARGET_COLUMN]]
...
class LsacParams(object):
  ...
  NUMERICAL_COLUMNS = (
      'lsat', 'DOB_yr', 'fulltime', 'decile1', 'decile1b', 'decile3',
  )

  CATEGORICAL_COLUMNS = (
      'race1', 'cluster', 'fam_inc',
  )

  PROTECTED_GROUPS = (
      'race1_white', 'race1_non_white', 'male', 'female',
  )
  ...
  OMIT_COLUMNS = (
      ...
      'race1_asian', 'race1_black', 'race1_hisp', 'race1_other',
  )
...
def _collect_protected_dataframes(dataframe, protected_groups):
  ...
  protected_groups = [
      (g,) if isinstance(g, str) else g for g in protected_groups]
